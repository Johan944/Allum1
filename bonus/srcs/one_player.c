...
** Last update Sun Feb 21 16:01:07 2016 johan ganem--brunet
*/

#include "../include/allum1.h"

t_sdl	check_values(t_sdl sdl)
{
  int	i;
...
t_sdl	detect_buttons(t_sdl sdl)
{
...
t_sdl	remove_matches(t_sdl sdl)
{
  int	i;
  int	j;
...
t_sdl	disp_screen(t_sdl sdl, int i)
{
  sdl.validate = 0;
  if (i == 1)
    {
...
int	one_player(t_sdl sdl, t_sound sound)
{
  ...
  while (sdl.exit == 0 && sdl.validate == 0)
	...
	sdl = detect_buttons(sdl);
      SDL_Flip(sdl.screen);
    }
  sdl = remove_matches(sdl);
  if (sdl.errors == -1)
    return (error_functions("SDL_BlitSurface"));
  if (count_matches(sdl) == 0)
    return (win_ai(sdl, sound));
  ia(sdl, sound);
}
